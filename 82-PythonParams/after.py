...
class VideoSearchQuery:
    ...
    def __post_init__(self) -> None: # Validation aded away from method to dataclass init
    ...
    @property
    def cache_key(self) -> str: # A property to generate a cache key based on the search parameters
        return (
            f"{self.query}:"
            f"{self.category}:"
            f"{self.sort_by}:"
