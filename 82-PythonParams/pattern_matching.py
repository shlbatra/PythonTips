# ---------------------------------------------------------------------------
# Structural pattern matching (match/case) — intuition primer
# ---------------------------------------------------------------------------
# `if` asks "is this value equal to that?" — `match` asks "does this thing have
# this shape, and while checking, hand me the pieces I want."
#
# Step 0 — match is a smarter if/elif (cases run top-to-bottom, first wins):
#     match x:
#         case 0: return "zero"
#         case 1: return "one"
#         case _: return "many"      # _ is the wildcard, "anything else"
#
# Step 1 — a bare name CAPTURES (binds) instead of comparing:
#     match x:
#         case 0:     print("zero")
#         case other: print(f"got {other}")   # puts x into `other`
#
# Step 2 — match on an object's SHAPE. Inside a case, Point(...) is NOT
# constructing — it's a question: "is it a Point, do these fields match?"
#     match p:
...
#   -> field=literal  is a GUARD   (must be equal)
#   -> field=variable is a CAPTURE (bind it out)
#
# Real usage below: build_search_query() guards on sort_by while capturing
# max_results; track_search() guards on region+include_shorts while capturing
# query — the type check, value conditions, and extraction in one line.
# ---------------------------------------------------------------------------
