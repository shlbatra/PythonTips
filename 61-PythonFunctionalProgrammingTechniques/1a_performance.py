...
def quick_sort(data: list[int]) -> list[int]:
    if len(data) <= 1:
        return data.copy()  # Work with a copy to ensure immutability
...
# Recursive vs Iterative approaches: Recursive solutions are often more elegant and easier to understand, but can lead to stack overflow for large datasets. Iterative solutions are generally more memory efficient and can handle larger datasets without risk of stack overflow.
def main() -> None:
