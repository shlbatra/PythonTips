...
class BadUserAccount:
    ...
    async def username(self) -> str:
        # Looks cheap, but does I/O every time - break property promise with IO calls
...
class UserAccount:
    ...
    async def load(cls, user_id: int, repo: UserRepository) -> "UserAccount":
        username, email, status = await asyncio.gather( # gets all data in parallel as async
            repo.fetch_username(user_id),
            repo.fetch_email(user_id),
            repo.fetch_status(user_id),
        )
        return cls( # create instance from data
            user_id=user_id,
            username=username,
            email=email,
            status=status,
    ...
    @property # Non async property - better way to do it.
...
async def main() -> None:
    ...
    account.email = "new-email@arjancodes.com" # Sync property - no side effect, just setting value, not doing work, not breaking property contract, local and explicit method to save to database
