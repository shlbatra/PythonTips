...
def read_airports_dataset(path: Path, filename: str) -> pd.DataFrame: # bunch of meta info so doesnt infer type correctly
...
def remove_metadata_information(path: Path, filename: str) -> pd.DataFrame: # skip first two rows so remove meta rows
...
def main() -> None:
    dataset_path = Path().absolute() / "47-Pandas/pandas" / "datasets"
    ...
    airports_types_converted = airports_raw.astype(mapping_types_conversion) # manually provide types here
