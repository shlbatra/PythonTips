...
class Options(TypedDict, total=False): # total=False makes all fields optional
    age_limit: int
    valid_land_codes: list[str]

# Group arguments into Objects or Data Classes when there are multiple related parameters
def is_eligible(data: Customer, options: Options) -> bool:
    age_limit = options.get("age_limit", 0)
    valid_land_codes = options.get("valid_land_codes", ["+91"]) # so its immutable
...
class OptionsV2:
    age_limit: int = 0
    valid_land_codes: list[str] = field(default_factory=lambda: ["+91"]) # Lambda called each time an instance is created, new list each time, immutable
...
def main() -> None:
    ...
    is_eligible(customer, {"age_limit": 18, "valid_land_codes": ["+91", "+92"]}) # dict
    is_eligible_v2(customer, OptionsV2(valid_land_codes=["+91", "+92"])) # dataclass based instance
