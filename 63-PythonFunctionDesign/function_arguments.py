...
# Mutable default argument bad practice as shared by all calls
...
# But, the timestamp is calculated only once when the module is loaded . Wrong practice - timestamp: float | None = time.time()
...
# float | None is equivalent to Optional[float] = None
def log_message(
    message: str, timestamp: float | None = None, level: str = "INFO" # default level is INFO; Non default before default. Similar in dataclass non default fields should come before default fields
) -> None:
