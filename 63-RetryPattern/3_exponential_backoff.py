...
def retry(operation, retries: int = 3, delay: float = 1.0, backoff: float = 2.0):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt == retries:
                raise
            sleep_time = delay * (backoff ** (attempt - 1)) # Exponential backoff calculation to allow longer waits between retries
...
def main() -> None:
    joke: str = retry(fetch_joke, retries=5, delay=1.0, backoff=2.0)
    print(joke)
