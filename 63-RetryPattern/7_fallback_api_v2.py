...
# Fixes issue with decorator approach where only one backup API can be used, here we can chain multiple functions for backups
def get_joke() -> str:
    return retry([fetch_main_api] * 3 + [fetch_backup_api]) # 3 retries for main API, then fallback to backup
