...
def generate_customer_report(
    ...
    for customer in customers:
        ...
        for order in orders: # Red flag - first customer loop, then orders loop (joining) - model data better -> group_orders_by_customer instead dict of cust_id -> [Orders]
            if order.customer_id == customer.id:
                if order.status == "paid":
                    order_total = Decimal("0")

                    for item in order.items:
                        order_total += item.price * item.quantity # move out out report generation as at order level - order class property order_total
