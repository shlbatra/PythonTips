...
class Order:
    ...
    @property
    def total(self) -> Decimal: # calculate at order level
...
# group orders by customer to avoid nested loops
...
# Methods with single responsibility - paid_orders, apply_discount, build_customer_summary, generate_customer_report
...
def build_customer_summary(
    ...
    totals = [apply_discount(customer, order.total) for order in paid_orders(orders)] # call other methods to get paid orders and apply discount
...
def generate_customer_report(
    ...
    orders_by_customer = group_orders_by_customer(orders) # Build summary here to avoid nested loops
    summaries: list[CustomerSummary] = []

    for customer in customers:
        customer_orders = orders_by_customer.get(customer.id, []) # get all orders for customers
