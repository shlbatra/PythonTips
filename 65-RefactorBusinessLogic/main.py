...
def approve_order(order: Order, user: User) -> str:
    ...
    ] # Use list of lambdas to clearly separate each rejection rule and also seperate definition from execution

    if any(rule() for rule in rejection_rules): # Execute each rejection rule here
        return "rejected"
    return "approved"
