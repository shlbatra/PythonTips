...
# --- Edge cases. that failed changing the logic---------------------------------------------------------------

# is admin special case 
def test_admin_and_premium():
    user = make_user(is_admin=True, is_premium=True, region="EU")
    order = make_order(amount=1500, has_discount=False, region="EU", currency="USD")
    assert approve_order(order, user) == "rejected"

# premium and not trial user
def test_premium_low_value_bulk_order_non_trial_discount():
    user = make_user(is_premium=True, is_trial=False)
    order = make_order(amount=500, type="bulk", has_discount=True)
    assert approve_order(order, user) == "approved"

# No check for negative item price if not in EU
def test_eu_eur_negative_item_price():
    user = make_user(is_premium=True, region="EU")
    order = make_order(amount=1500, has_discount=False, region="EU", currency="EUR", items=[Item("Faulty item", -20)])
    assert approve_order(order, user) == "approved"