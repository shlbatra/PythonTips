...
class OrderEncapsulationNoInformationHiding:
    ...
    #read/write from outside - no info hiding. Make change to payment status 
    #then change methods as well
...
class OrderEncapsulationAndInformationHiding:
    ...
    is the is_paid method, you need no knowledge of how status is represented 
    (that information is 'hidden')."""
    #payment status info is hidden here, outside no info changing if using the same class
    _payment_status: PaymentStatus = PaymentStatus.PENDING
