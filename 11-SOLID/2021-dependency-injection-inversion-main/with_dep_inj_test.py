#coverage html -> to check how many lines of codes have been unit tested
import unittest
...
class PaymentProcessor_TestCase(unittest.TestCase):
    ...
    def test_payment_fail(self):
        ...
        with patch('builtins.input', return_value="1234567"):
            p = PaymentProcessor(auth)
            order = Order()
            self.assertRaises(Exception, p.pay, order) #expect exception when raise pay method with order sent
