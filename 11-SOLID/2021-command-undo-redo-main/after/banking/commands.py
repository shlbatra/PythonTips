#create common transactions, match with protocol class transaction
...
#batch accepts list of commands 
@dataclass
class Batch:
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            for command in reversed(completed_commands): #undo all commands that are completed, go back in time from latest so use reversed
                command.undo()
            raise

    def undo(self) -> None:
        for command in reversed(self.commands): #undo all commands -> reverse from latest all commands , add exceptions here but complicate it - how undo undo ?
            command.undo()

    def redo(self) -> None:
        for command in self.commands: #redo all commands , similar to execute 
            command.redo()
