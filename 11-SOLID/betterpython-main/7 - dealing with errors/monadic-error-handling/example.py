...
def fetch_blog(blog_id) -> Result['Blog', Exception]: #return success or error part here
    return flow(
        blog_id, #parameter
        fetch_blog_from_db, #in sequence, call functions, returns Result container
        bind(blog_to_dict), #bind -> converts regular fn that needs a parameter to result container with success track having the parameters
        bind(verify_access)
    ) #two track railway

@safe #decorator converts return value for these fns to Result[success,failure]
