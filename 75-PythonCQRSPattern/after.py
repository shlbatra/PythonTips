...
async def project_ticket(db: Database, ticket_id: str) -> None:
    ...
    read_doc: dict[str, Any] = {
        ...
        "preview": make_preview(doc.get("message", "")), # Only compute preview once during projection
        "has_note": bool(note),
    }
...
async def create_ticket(cmd: CreateTicket) -> dict[str, str]:
    ...
    await project_ticket(db, ticket_id)  # synchronous for demo; update projection
    return {"id": ticket_id}
...
async def list_tickets(
    ...
    cursor = (
        db[READS_COLL] # Use the read model collection
        .find(
            query,
            projection={
                "subject": 1,
...
# Project the dashboard data in a way that's optimized for the dashboard query patterns, without affecting the write model or the list endpoint performance.
# Can be scheduled to run periodically, or triggered by relevant events (e.g. status updates) for near real-time analytics.
@app.get("/dashboard")
async def dashboard() -> dict[str, int]:
    ...
    cursor = await db[READS_COLL].aggregate(pipeline) # Just reference the read model for analytics
