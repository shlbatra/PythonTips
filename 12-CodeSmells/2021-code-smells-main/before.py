...
class Employee:
    ...
    name: str
    role: str  # code smell 1 -> string represent anything so broad type (use enum)
    vacation_days: int = 25

    def take_a_holiday(self, payout: bool) -> None:
        # Code Smell 6 -> bool flag for single holiday or payout.
        # fn does 2 diff things based on flag (combine responsibility here)
        # so long function - low cohesion so split method into two
...
class HourlyEmployee(Employee):
    # Code smell 4 -> Use vague identifiers or not have units, ex - amount
    # change name to hours_worked (included units)
...
class Company:
    ...
    # Code Smell 2 - all find methods have similar code -> duplicate code
    def find_managers(self) -> List[Employee]:
        """Find all manager employees."""
        # Code smell 3 -> use built in python fns. Below can be done using list comprehension
        managers = []
    ...
    def pay_employee(self, employee: Employee) -> None:
        """Pay an employee."""
        # Code smell 5 -> avoid using isinstance
        # extend if else if add another subtype of employee
        # direct dependency between class and sub class -> lot of coupling
        # add pay method to each sub class type -> salaried and hourly and call here
...
# code smell 7 -> catch and then not ignore excpetions, ignore syntax or keyboard interruptions
# catch exception of particular type and deal with it
"""
try:
    <code>
except Exception:
...
#fix to remove it as not using it here anyway             
"""
